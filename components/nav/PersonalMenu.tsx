  ...
  return (
    <div className="w-[900px] p-8" ref={ref as LegacyRef<HTMLDivElement>}>
      ...
      <div className="grid grid-cols-[250px,1fr] items-start gap-8">
        <div>
          <div className={`sm rounded-xl mb-4 p-4 ${personalSelectedIndex === 0 ? 'bg-[#F5F4F1]' : ''}`} onMouseEnter={() => setPersonalSelectedIndex(0)} style={{ transition: '.3s' }}>
            <h6 className="">Save</h6>
            <div className='text-sm'>Grow your savings securely with unity accounts and deposits</div>
          </div>
          <div className={`sm rounded-xl mb-4 p-4 ${personalSelectedIndex === 1 ? 'bg-[#F5F4F1]' : ''}`} onMouseEnter={() => setPersonalSelectedIndex(1)} style={{ transition: '.3s' }}>
            <h6 className="">Borrow</h6>
            <div className='text-sm'>Get instant loans with interest rates in your interest</div>
          </div>
          <div className={`sm rounded-xl mb-4 p-4 ${personalSelectedIndex === 2 ? 'bg-[#F5F4F1]' : ''}`} onMouseEnter={() => setPersonalSelectedIndex(2)} style={{ transition: '.3s' }}>
