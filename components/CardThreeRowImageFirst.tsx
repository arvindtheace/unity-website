        ...
        <div className="featured-blog-carousel">
          ...
          >
            <div 
              ...
              style={{ backgroundImage: `url(${info[currentSlide].article.value.data.image})` }}
            >
              <Badge className='absolute top-4 left-4'>Featured</Badge>
            </div>
            <div>
              <h4 className='font-semibold mb-4'>{info[currentSlide].article.value.data.title}</h4>
              <div className="sm mb-6">
                <p>{info[currentSlide].article.value.data.excerpt}</p>
              </div>
              <div className='text-sm text-gray-500 mb-6'>{info[currentSlide].article.value.data.readTime} min read&nbsp;&nbsp;|&nbsp;&nbsp;{formatDistanceToNow(info[currentSlide].article.value.data.date, { addSuffix: true })}</div>
              <Button
                text="Read Blog"
                type="link"
                href={`/blog/${info[currentSlide].article.value.data.slug}`}
                icon="arrow-right"
              />
            </div>
          </div>
          <div className="flex items-center justify-center space-x-3 mt-8">
            ...
            {info?.map((blog: any, index: number) => (
              <button
                key={blog.id}
